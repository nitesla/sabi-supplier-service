...
public interface SupplyRequestResponseRepository extends JpaRepository<SupplyRequestResponseEntity, Long> {
    List<SupplyRequestResponseEntity> findByIsActiveOrderByIdDesc(Boolean isActive);
    ...
    @Query("SELECT s FROM SupplyRequestResponseEntity s WHERE ((:supplyRequestId IS NULL) OR (:supplyRequestId IS NOT NULL AND s.supplyRequestId = :supplyRequestId))order by s.id DESC ")
