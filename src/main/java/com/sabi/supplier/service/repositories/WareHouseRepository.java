...
public interface WareHouseRepository extends JpaRepository<WareHouse, Long>, JpaSpecificationExecutor<WareHouse> {
    ...
    @Query("SELECT c FROM WareHouse c WHERE ((:supplierId IS NULL) OR (:supplierId IS NOT NULL AND c.supplierId = :supplierId))" +
            ...
            "AND((:contactPerson IS NULL) OR (:contactPerson IS NOT NULL AND c.contactPerson = :contactPerson))" +
            "AND((:contactPhone IS NULL) OR (:contactPhone IS NOT NULL AND c.contactPhone = :contactPhone))" +
            "AND((:contactEmail IS NULL) OR (:contactEmail IS NOT NULL AND c.contactEmail = :contactEmail))" +
            "AND((:longitude IS NULL) OR (:longitude IS NOT NULL AND c.longitude = :longitude))" +
            "AND((:latitude IS NULL) OR (:latitude IS NOT NULL AND c.latitude = :latitude))" +
            ...
            "AND((:name IS NULL) OR (:name IS NOT NULL AND c.name = :name))" +
            "AND((:isActive IS NULL) OR (:isActive IS NOT NULL AND c.isActive = :isActive))" +
            " AND ((:address IS NULL) OR (:address IS NOT NULL AND c.address = :address)) order by c.id desc "
    )
